...
<script type="text/javascript">
    function showhide(element){
        ...
        if (element.style.display == "block") {
            document.getElementById("label").text = "Show ▼"
            element.style.display = "none"
        }
            
        else {
            document.getElementById("label").text = "Hide ▶"
...
class HiddenCodeBlock(CodeBlock):
    ...
    def run(self):
        ...
        hcb['label'] = self.options.get('label', "Show ▼" if hcb['starthidden'] else "Hide ▶")
...
def visit_hcb_html(self, node):
    ...
    divheader = ("""<a id="label" href="javascript:showhide(document.getElementById('{divname}'))">"""
                 """{label}</a><br />"""
